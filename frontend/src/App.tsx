...
function App() {
  ...
  const [activeLeftTab, setActiveLeftTab] = useState<'rag' | 'agentic'>('rag')
        ...
        <header className="flex flex-col gap-2">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="badge badge-lg badge-primary uppercase">
                {activeLeftTab === 'rag' ? 'RAG' : 'Agentic RAG'}
              </span>
            </div>
      ...
      <div className="app-layout mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col gap-5 lg:h-full lg:gap-7">
        <div className="app-columns grid min-h-0 flex-1 grid-cols-1 gap-5 lg:h-full lg:grid-cols-5 lg:items-stretch">
          <section className="card control-panel flex h-full min-h-0 flex-col border border-base-300 bg-base-100 shadow-2xl lg:col-span-2 lg:overflow-hidden">
            <div className="card-body flex min-h-0 flex-1 flex-col gap-4 text-sm lg:overflow-hidden">
              <div role="tablist" className="tabs tabs-boxed tabs-lg w-full">
                <button
                  role="tab"
                  className={`tab font-semibold ${activeLeftTab === 'rag' ? 'tab-active' : ''}`}
                  aria-label="Switch to RAG panel"
                  aria-selected={activeLeftTab === 'rag'}
                  onClick={() => setActiveLeftTab('rag')}
                >
                  RAG
                </button>
                <button
                  role="tab"
                  className={`tab font-semibold ${activeLeftTab === 'agentic' ? 'tab-active' : ''}`}
                  aria-label="Switch to Agentic RAG panel"
                  aria-selected={activeLeftTab === 'agentic'}
                  onClick={() => setActiveLeftTab('agentic')}
                >
                  Agentic RAG
                </button>
              </div>
