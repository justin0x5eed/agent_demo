...
function App() {
  ...
  const [activeControlTab, setActiveControlTab] = useState<'rag' | 'agentic'>('rag')
      ...
      <div className="app-layout mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col gap-5 lg:h-full lg:gap-7">
        <div className="app-columns grid min-h-0 flex-1 grid-cols-1 gap-5 lg:h-full lg:grid-cols-5 lg:items-stretch">
          <section className="card control-panel flex h-full min-h-0 flex-col border border-base-300 bg-base-100 shadow-2xl lg:col-span-2 lg:overflow-hidden">
            <div className="card-body flex min-h-0 flex-1 flex-col gap-3 text-sm lg:overflow-hidden">
              <div className="tabs tabs-lifted tabs-lg">
                <button
                  className={`tab gap-2 px-4 ${activeControlTab === 'rag' ? 'tab-active [--tab-bg:theme(colors.base-100)] [--tab-border-color:oklch(var(--bc)/0.2)]' : ''}`}
                  onClick={() => setActiveControlTab('rag')}
                  type="button"
                  aria-current={activeControlTab === 'rag' ? 'page' : undefined}
                >
                  RAG
                </button>
                <button
                  className={`tab gap-2 px-4 ${activeControlTab === 'agentic' ? 'tab-active [--tab-bg:theme(colors.base-100)] [--tab-border-color:oklch(var(--bc)/0.2)]' : ''}`}
                  onClick={() => setActiveControlTab('agentic')}
                  type="button"
                  aria-current={activeControlTab === 'agentic' ? 'page' : undefined}
                >
                  Agentic RAG
                </button>
              </div>

              <div className="tab-content min-h-0 flex-1 overflow-hidden rounded-box border border-base-300 bg-base-100/70 p-3 shadow-inner">
                <div
                  className={`${activeControlTab === 'rag' ? 'flex' : 'hidden'} min-h-0 flex-1 flex-col gap-3 lg:overflow-y-auto`}
                >
                  {renderControlPanelContent()}
                </div>
                <div
                  className={`${activeControlTab === 'agentic' ? 'flex' : 'hidden'} min-h-0 flex-1 flex-col gap-3 lg:overflow-y-auto`}
                >
                  {renderControlPanelContent()}
                </div>
              </div>
